...
@TargetUrl("http://club.xdnice.com/thread-\\d{7}-1-1.html")
@HelpUrl("http://club.xdnice.com/forum-113-\\d+.html")
public class HaowangJobInfo extends BaseNoticeMessage implements AfterExtractor {

    @ExtractBy("//span[@id=\"thread_subject\"]/text()")
    private String myTitle;

    //@ExtractBy("//div[@class=\"comiis_viewb_box\\s+c1\"]//regex(\\d+-\\d+-\\d+\\s+\\d+:\\d+:\\d+)")
...
//    private String author;
    ...
    //@ExtractBy("//div[@class=\"comiis\\_viewbt\\_box\\s+c1\"]//regex('\\d+-\\d+-\\d\\s+\\d+:\\d+')")
    @ExtractBy(value = "(\\d{4}-\\d{1,2}-\\d{1,2} \\d{2}:\\d{2}:\\d{2})",type = ExtractBy.Type.Regex)
